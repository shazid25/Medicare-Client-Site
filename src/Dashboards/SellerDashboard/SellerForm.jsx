...
const SellerForm = () => {
  ...
  const {
    ...
    watch,
    reset,
    setValue, // Added setValue for better field updates
    setError, // Added setError for server error handling
  ...
  // Auto discount calculation - FIXED: Use setValue instead of reset
  useEffect(() => {
    const origPrice = parseFloat(originalPrice) || 0;
    const disc = parseFloat(discount) || 0;
    
    if (origPrice > 0 && disc >= 0 && disc <= 100) {
      const discountedPrice = origPrice - (origPrice * disc) / 100;
  ...
  }, [originalPrice, discount, setValue]); // Added setValue to dependencies
    ...
    try {
      ...
      if (!user || !user.email) {
        Swal.fire({
          ...
          text: "You must be logged in to post a medicine.",
        });
        return;
      }
      ...
      const { data } = await axios.post("https://medicare-sever-site.vercel.app/medicine", payload, {
        headers: {
      ...
      if (err.response?.status === 403) {
        Swal.fire({
          icon: "error",
          title: "Access Denied",
          text: "You do not have permission to perform this action. Please ensure you are logged in and have seller privileges.",
        });
        // Optionally, set a form error for feedback
        ...
        Swal.fire({
          ...
          text:
            ...
            err.message ||
            "Could not post medicine. Please try again.",
        });
      }
    }
  };

  // The JSX return statement remains exactly the same as your original code
  return (
