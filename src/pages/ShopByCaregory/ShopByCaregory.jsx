...
const ShopByCaregory = () => {
  const { id } = useParams(); // e.g. "Cardiology"
  ...
  useEffect(() => {
    const fetchMedicines = async () => {
      try {
        ...
        const withoutDetails = data.map(({ details, ...rest }) => rest);
        setMedicines(withoutDetails);
      } catch (error) {
        console.error("Failed to fetch category medicines:", error);
      }
    };
    fetchMedicines();
  }, [id]);
    ...
    <section className="container mx-auto px-4 mb-12">
      ...
      {medicines.length === 0 ? (
        <p className="text-gray-500">
          No medicines available in {id} category.
        </p>
      ) : (
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="bg-gradient-to-r from-blue-500 to-teal-500 hover:from-blue-600 hover:to-teal-600 text-white px-3 py-2 rounded-full text-sm font-medium transition-all duration-300"
                  >
                    Add to Cart
                  </button>
                </div>
