  ...
  useEffect(() => {
    const fetchMedicine = async () => {
      try {
        ...
        setMedicine(response.data.data); // <-- Change here: access response.data.data
        ...
        if (response.data.data.details) {
          ...
          const interval = setInterval(() => {
            setTypedText((prev) => prev + response.data.data.details[index]);
            index++;
            if (index >= response.data.data.details.length) clearInterval(interval);
          }, 25);
